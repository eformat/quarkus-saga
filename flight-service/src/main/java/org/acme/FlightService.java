...
public class FlightService {
    ...
    @Path("buy")
    @Operation(operationId = "recreate", summary = "buy flights", description = "This operation buys you a ticket to fly.", deprecated = false, hidden = false)
    @Tag(name = "Admin")
    @APIResponse(responseCode = "200", description = "Action successful", content = @Content(schema = @Schema(implementation = Response.class), examples = @ExampleObject(name = "example", value = "{\"message\": \"Flight Booked #1.\"}")))
    ...
    public Response buy(@HeaderParam(LRA_HTTP_CONTEXT_HEADER) URI lraId, Purchase purchase) {
        ...
        purchase.setItem("FLIGHT# ".concat(purchase.getItem()));
        log.info(">>> Booking flight for LRA {} and {}", lraId, purchase);
        try {
        ...
        JsonObject response = new JsonObject().put("message", "Flight Booked LRA #" + lraId);
    ...
    public Response compensate(@HeaderParam(LRA_HTTP_CONTEXT_HEADER) URI lraId) {
        ...
        try {
            Purchase purchase = new Purchase(lraId.toASCIIString(), "CANCELLED FLIGHT");
            purchases.send(KafkaRecord.of(purchase.getPurchaseKey(), purchase));
        } catch (Exception ex) {
            JsonObject response = new JsonObject().put("message", "Caught Exception: " + ex);
            return Response.serverError().entity(response).build();
